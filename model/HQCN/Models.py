...
class HQCN(nn.Module):
    ...
    def __init__(
        ...
        """
        vocab  —— pretrain word embedding class
        """
        ...
        # self.class_weight_reform = torch.FloatTensor([35378, 298660, 598312, 267352]).to(self.device) # AOL
        self.class_weight_reform = torch.FloatTensor([1, 1, 1, 1]).to(self.device)
        ...
        self.ranker = nn.Linear(6, 1, 1)  # 删了feature
    ...
    def forward(self, queries, documents, can_index, wss_label, features=[]):
        ...
        loss_reform = reform_loss_fct(reform_logits.view(-1, self.reform_type), wss_label.view(-1).long())
        # ----------------------reformulation classifier -----------------------
        ...
        # feat_score = torch.tanh(self.feat_linear(features.float())).squeeze(-1)
        scores = torch.stack((mean_rep_score, retained_score, added_score, removed_score, mean_score, ad_hoc_score
                              # , feat_score
                              ), -1)

        score =  torch.tanh(self.ranker(scores)).squeeze(-1)
