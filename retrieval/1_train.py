    ...
    for epoch in range(args.epoch):
        ...
        for idx_,  batch_ in tqdm(enumerate(train_dataloader)):
            ...
            for kkkkk in range(this_batch_size):
                ...
                rec_poi_list_id = batch_[3][kkkkk]
                ...
                for rand_neg_poid in set_rand_neg_poid:
                    ...
                    batch_rand_neg_poi_context.append(
            ...
            ### click poi 
            batch_pos_poi_context = tokenizer(batch_pos_poi_context, padding=True, return_tensors='pt')
            ##### rand neg poi
            batch_rand_neg_poi_context = tokenizer(batch_rand_neg_poi_context, padding=True, return_tensors='pt')
            ...
            with autocast():
                ...
                pos_poi_context_emb = itmodel.module.cls_(
                    batch_pos_poi_context['input_ids'].to(device), 
                    batch_pos_poi_context['attention_mask'].to(device), 
                    batch_pos_poi_context['token_type_ids'].to(device),
                )
                rand_neg_poi_context_emb = itmodel.module.cls_(
                    batch_rand_neg_poi_context['input_ids'].to(device), 
                    batch_rand_neg_poi_context['attention_mask'].to(device), 
                    batch_rand_neg_poi_context['token_type_ids'].to(device),
                )
                ...
                pos_neg_diff = scores_pos.mean() - torch.cat([scores_inbatch_neg,scores_rand_neg,], dim=1).mean()
...
if __name__ == "__main__":
    ...
    # apex
    parser.add_argument('--mix_amp', default=1, type=int, help='node rank for distributed training')
    ...
    # checkpoint
    parser.add_argument('--checkpoint_path', type=str, default='checkpoints-temp0.1/')
