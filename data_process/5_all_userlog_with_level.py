...
def sampled_split_data(args, dict_query_level, dict_level_ratio):
    ...
    f_t = open(args.path_clicklog_sample, 'w')
    w_t = csv.writer(f_t, delimiter='\01')
    

    file = open(args.path_filter_log, mode='r')
    reader = csv.reader(file, delimiter='\01')

    count_t = [[0, 0] for i in range(len(dict_level_ratio))] 
    ...
    for index, row in tqdm(enumerate(reader)):
        ...
        if index >= 1:
            ...
            if prob <= real_prob / (len(dict_level_ratio) * level_ratio):
                count_t[query_content_level][0] += 1
                w_t.writerow(row)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path_clicklog_sample', type=str, default="5_all_userlog_with_level.csv")
    ...
    parser.add_argument('--sample_prob', type=float, default=1, help='')
